...
あらたにRスクリプトを作成し、`data_viz.R`という名前をつけて`r_practice`フォルダに保存する。
...
- `ggsave`という関数で、ファイルパスを指定して保存
  - `file`という引数に、`"fig/musashi_students_2025.png"`と指定
    - `fig`ディレクトリの下に`musashi_students_2025`という名前で`png`形式ファイルとして保存するという意味
...
ggsave(file="fig/musashi_students_2025.png", plot=plot_634, device="png")
