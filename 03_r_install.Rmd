...
`j:`というドライブの下の`abezemi`というフォルダの下にある`r_practice`というフォルダにある`kadai1.R`というファイルがあるとする。そのファイルのpathは`j:/abezemi/r_practice/kadai1.R`となる。
...
- プロジェクトが作成されると、作成したディレクトリにはプロジェクト名.Rprojという名前のファイルが作成される
 - 今回はabezemiの下に`r_practice`フォルダが作成され、`r_practice.Rproj`というファイルができる
- プロジェクトの設定などを保存
...
2. "Untitled1"というRスクリプトが作成されるので名前をつけて保存する
  - File -> Save As... -> `practice.R`という名前をつけて保存
  - もしくは右上のフロッピー![](fig/floppy.png)を押して`practice.R`という名前をつけて保存 
