...
knitr::include_graphics("docs/r_basics/fig/gyoretsu.png")
```

行と列の覚え方

```{r  label = gyouretsu2, echo = FALSE, fig.cap = "", out.width = '80%'}
knitr::include_graphics("docs/r_basics/fig/gyoretsu2.png")
```
...
csvを読み込むには`read_csv()`関数を使う。この関数は`readr`パッケージ含まれており、tidyverseパッケージ群の一つである。

1. `gapminder.csv`というデータファイルを<a href="https://keita43a.github.io/regression_tutorial/data/gapminder.csv" download="gapminder.csv">こちら</a>からダウンロードするか、阿部ゼミのGoogle Classroomからダウンロードする。

2. `r_renshu`フォルダに`data`フォルダを作成し、そこに`gapminder.csv`ファイルを保存する。
...
武蔵大学経済学部の学生数データを<a href="https://keita43a.github.io/regression_tutorial/data/musashi_keizai_students_2023.xlsx" download="musashi_keizai_students_2023.xlsx">こちら</a>からダウンロードして、`data`フォルダに保存する。

以下のように、`readxl`パッケージに入っている`read_excel`関数でエクセルファイルを読み込む。`readxl`パッケージは`tidyverse`パッケージ群に入っている。
...
```{r, echo =FALSE}
data_634_long = readxl::read_excel("docs/r_basics/data/musashi_keizai_students_2023.xlsx") %>%
...
knitr::include_graphics("docs/r_basics/fig/dplyr_logo.png")
```
...
```{r}
## エクセルからデータを読み込み
data_634_jinbun <- readxl::read_excel("docs/r_basics/data/musashi_jinbun_students_2023.xlsx") 
...
```{r}
## エクセルからデータを読み込み
data_634_keizai_teiin <- readxl::read_excel("docs/r_basics/data/musashi_keizai_teiin_2023.xlsx") 
