...
knitr::include_graphics("r_basics/fig/gyoretsu.png")
```

行と列の覚え方

```{r  label = gyouretsu2, echo = FALSE, fig.cap = "", out.width = '80%'}
knitr::include_graphics("r_basics/fig/gyoretsu2.png")
```
...
1. `gapminder.csv`というデータファイルをGoogle Classroomからダウンロードする。
...
```{r}
## エクセルからデータを読み込み
data_634 <- readxl::read_excel("r_basics/data/musashi_keizai_students_2023.xlsx") 
...
```{r, echo =FALSE}
data_634_long = readxl::read_excel("r_basics/data/musashi_keizai_students_2023.xlsx") %>%
...
knitr::include_graphics("r_basics/fig/dplyr_logo.png")
```
...
```{r}
## エクセルからデータを読み込み
data_634_jinbun <- readxl::read_excel("r_basics/data/musashi_jinbun_students_2023.xlsx") 
...
```{r}
## エクセルからデータを読み込み
data_634_keizai_teiin <- readxl::read_excel("r_basics/data/musashi_keizai_teiin_2023.xlsx") 
